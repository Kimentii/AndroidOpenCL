// Work group size: 16x4
// Work item process 1x4 pixel
// Work group scope: 64x64
// Each group loads 64x64 pixels and gives (64-filter_size/2)x(64-filter_size/2)
// result. So data is loading in local_work not from (64n)x(64m) but from
// ((64-filter_size/2)n)x((64-filter_size/2)m)
// Borders are simply ignored, you can see it on result image
...
__kernel void blur_filter(
    ...
    for (int i = 0; i < ROWS_PER_ITEM; i++) {
        if ((gy*GROUP_RESULT_H + ROWS_PER_ITEM*y + i) < h) {
            ...
            // You can set pixels manually here for debug/fun purpose
    ...
    for (int i = 0; i < ROWS_PER_ITEM; i++) {
        if ((x < GROUP_RESULT_W) && ((gx*GROUP_RESULT_W + x) < w) && ((gy*GROUP_RESULT_H + y*ROWS_PER_ITEM + i) < h)) {
            ...
            // Here you can add some additional effects

            // Overpowering red color with saturation
            // uint overpowered_red = char_result.s2 * 2;
            // if (overpowered_red > 255) overpowered_red = 255;
            // char_result.s2 = overpowered_red;

            // Overpowering red color with overflow
