...
static const char* program_src = ""
        ...
        "\n"
        "\t__local uchar local_work[LOCAL_W * 4 * LOCAL_H];\n"
        "\tlong local_work_offset = GROUP_RESULT_H*gy*w + GROUP_RESULT_W*gx*4;\n"
        "\tfor (int i = 0; i < ROWS_PER_ITEM; i++) {\n"
        "\t\tif ((gy*GROUP_RESULT_H + ROWS_PER_ITEM*y + i) < (h + BORDER_WIDTH*2)) {\n"
        "\t\t\tuchar4 pixel = vload4((local_work_offset + y*ROWS_PER_ITEM*w + i*w)/4 + x, input_image);\n"
        "\t\t\tvstore4(pixel, (y*ROWS_PER_ITEM + i)*LOCAL_W/4 + x, local_work);\n"
        "\t\t}\n"
        "\t}\n"
...
//        "\t// filtering\n"
//        "\tfor (int i = 0; i < ROWS_PER_ITEM; i++) {\n"
//        "\t\tif ((x < ITEM_RESULT_W) && ((gx*ITEM_RESULT_W + x) < w*4) && ((gy*ITEM_RESULT_H + y*ROWS_PER_ITEM + i) < h)) {\n"
...
//        "\t\t\tvstore4(char_result, (i + y*ROWS_PER_ITEM)*ITEM_RESULT_W + x, local_result);\n"
//        "\t\t}\n"
//        "\t}\n"
//        "\t\n"
//        "\tw = 480;\n"
...
//        "\t\t\t}\n"
        "\t\t\t\tuchar4 output_pixel = (uchar4)vload4((y*ROWS_PER_ITEM + i)*w + x, local_work);\n"
        "\t\t\t\toutput_pixel.s0 = 200;\n"
        "\t\t\t\toutput_pixel.s1 = 200;\n"
        "\t\t\t\toutput_pixel.s2 = 200;\n"
        "\t\t\t\toutput_pixel.s3 = 200;\n"
        "\t\t\t\tvstore4(output_pixel, gy*LOCAL_H*w + y*ROWS_PER_ITEM*w + i*w + gx*LOCAL_W + x, output_image);\n"
